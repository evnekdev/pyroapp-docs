...
The workbook files are regenerated during every documentation deployment from the current example definitions. Excel will recalculate the PyroApp formulas when the installed add-in is available.
...
[**Download 02_PyroApp_DAT_Inspection_and_Editing_Al-Ca-O.xlsx**](../tutorial-assets/workbooks/02_PyroApp_DAT_Inspection_and_Editing_Al-Ca-O.xlsx)

Matching working datafile: [**Al-Ca-O_var.dat**](../tutorial-assets/datafiles/examples/Al-Ca-O_var.dat)
...
The SET sheet is disabled by default. It is designed around the published `*_var.dat` working copy so that a student does not accidentally overwrite the reference example.
...
The target Gibbs-energy values in the live template are deliberately labelled **illustrative**. They are there to demonstrate the mechanics of the optimization workflow; they are not presented as an assessed Ca-Al-O experimental dataset.
